...
class PathGenerator:
    ...
    def plot_dynamic_results(self, xx, xy, vel, omega, start, end):
        ...
        path_ax = fig.add_subplot(gs[:, 2:])
        path_ax.plot(start[0], start[1], marker='*',
                     color='g', markersize=15, label='Start')
        path_ax.plot(end[0], end[1], marker='*',
                     color='r', markersize=15, label='End')
        self.ppp.plot_all(path_ax)
        ...
        for i in range(len(xx)):
            ...
            veh = plt.Circle(
                (xx[i], xy[i]), self.config.vehicle_width/2, color='b', alpha=0.7, label='Robot')
            ...
            for j, obstacle in enumerate(self.ppp.dyn_obs_list):
                ...
                obs_padded[j] = mpatches.Ellipse(pos, x_rad_pad*2, y_rad_pad*2, angle/(
                    2*math.pi)*360, color='y', alpha=0.7, label='Padded obstacle')
    ...
    def run(self, graph_map, start, end):
        ...
        mng = og.tcp.OptimizerTcpManager(
            self.config.build_directory + os.sep + self.config.optimizer_name)
        mng.start()
        ...
        parameter_list = [self.config.q, self.config.qv, self.config.qtheta, self.config.lin_vel_penalty, self.config.ang_vel_penalty,
                          self.config.qN, self.config.qthetaN, self.config.cte_penalty, self.config.lin_acc_penalty, self.config.ang_acc_penalty]
        # generate costs to establish initial heading
        p_init_c = [0.0*temp for temp in parameter_list]
        # makes a list with large angular cost
        ...
        t_temp = time.time()
        path, _ = self.ppp.get_initial_guess(
        ...
        t_temp = time.time()
        x_ref, y_ref, theta_ref = self.mpc_generator.rough_ref(
            (start[0], start[1]), path[1:])
        ...
        dyn_constraints = [0.0] * self.config.Ndynobs * \
            self.config.ndynobs*self.config.N_hor
        # Avoid dividing by zero in MPC solver by init x radius and y radius to 1
        dyn_constraints[2::self.config.ndynobs] = [
            1.0] * self.config.Ndynobs*self.config.N_hor
        dyn_constraints[3::self.config.ndynobs] = [
            1.0] * self.config.Ndynobs*self.config.N_hor
        ...
        try:
            while (not terminal) and t < 500.0/self.config.ts:
                ...
                # set current state as initial state for solver
                x_init = states[-self.config.nx:]

                if len(self.ppp.original_obstacle_list):
                    # Create constraints from verticies
                    constraint_origin = self.ppp.find_closest_vertices(
                        (x_init[0], x_init[1]), self.config.Nobs, 0)
                    # Add radius to obstacle information
                    constraints = list(itertools.chain(
                        *[(x, y, self.config.vehicle_width/2 + self.config.vehicle_margin) for x, y in constraint_origin]))
                ...
                ub_idx = min(len(ref_points), idx+5 *
                             self.config.num_steps_taken)
                _, idx = self.ppp.get_closest_vert(
                    (x_init[0], x_init[1]), ref_points[lb_idx:ub_idx])
                idx += lb_idx  # idx in orignal list
                if (idx+self.config.N_hor >= len(x_ref)):
                    x_finish = end
                    tmpx = x_ref[idx:] + [end[0]] * \
                        (self.config.N_hor - (len(x_ref)-idx))
                    tmpy = y_ref[idx:] + [end[1]] * \
                        (self.config.N_hor - (len(y_ref)-idx))
                    tmpt = theta_ref[idx:] + [end[2]] * \
                        (self.config.N_hor - (len(theta_ref)-idx))
                else:
                ...
                # if any future step will be within braking "zone"
                if (idx+self.config.N_hor) >= len(x_ref)-brake_distances[0]/base_speed:
                    ...
                    if num_base_speed == 0:
                        ...
                        vel_ref = [vel for (vel, dist) in zip(
                            brake_velocities, brake_distances) if dist <= dist_to_goal]
                    else:
                        num_brake_vel = min(
                            len(brake_velocities), self.config.N_hor-num_base_speed)
                ...
                if(not establish_heading):
                    parameters = x_init+last_u+x_finish+last_u + \
                        parameter_list+vel_ref+constraints+dyn_constraints+refs
                else:
                    parameters = x_init+last_u+x_finish+last_u + \
                        p_init_c+vel_ref+constraints+dyn_constraints+refs

                try:
                    exit_status, solver_time = self.mpc_generator.run(
                        parameters, mng, self.config.num_steps_taken, system_input, states)
    ...
    def get_brake_vel_ref(self):
        ...
        brake_velocities = [base_speed-base_speed /
                            (brake_time_steps-1)*i for i in range(brake_time_steps)]
